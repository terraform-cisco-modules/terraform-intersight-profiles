...
data "intersight_search_search_item" "adapter_configuration" {
  for_each              = { for v in [0] : v => v if length(local.pb.adapter_configuration) > 0 && var.moids_policies == true }
  additional_properties = jsonencode({ "ObjectType" = "adapter.ConfigPolicy" })
}

data "intersight_search_search_item" "bios" {
  for_each              = { for v in [0] : v => v if length(local.pb.bios) > 0 && var.moids_policies == true }
  additional_properties = jsonencode({ "ObjectType" = "bios.Policy" })
}

data "intersight_search_search_item" "boot_order" {
  for_each              = { for v in [0] : v => v if length(local.pb.boot_order) > 0 && var.moids_policies == true }
...
data "intersight_search_search_item" "certificate_management" {
  for_each              = { for v in [0] : v => v if length(local.pb.certificate_management) > 0 && var.moids_policies == true }
